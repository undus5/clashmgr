...
clash_path=${runtime_dir}/${process_name}
...
get_help() {
    ...
    printf "Usage: ${program_name} <start|stop|status|update|set|get>\n"
    printf "Options:\n"
    format="%3s %-20s %s\n"
    ...
    printf "ENV:\n"
    printf "$format" "" "CLASH_CONF" "config file path environment variable"
}
...
get_status() {
    ...
    if [[ -z $external_controller ]]; then
        echo "no 'external-controller' attribute in ${runtime_dir}/config.yaml"
        return 1
    fi
...
update_clash() {
    if [[ -f $clash_update_path ]]; then
        ...
        mv -f $clash_update_path $runtime_dir
...
update_dashboard() {
    if [[ -d $dashboard_update_path ]]; then
        ...
        mv -f $dashboard_update_path $runtime_dir
        if [[ $? == 0 ]]; then
            printf "success\n"
        fi
    fi
...
update_geoip() {
    if [[ -f $geoip_update_path_path ]]; then
        ...
        mv -f $geoip_update_path_path $runtime_dir
        if [[ $? == 0 ]]; then
            printf "success\n"
        fi
    fi
...
make_backup() {
    cp -r $runtime_dir $runtime_old_dir
}
...
update() {
    ...
    if [[ -d $runtime_old_dir ]]; then
        echo "'runtime_old' exists, remove first."
        exit 1
    fi
